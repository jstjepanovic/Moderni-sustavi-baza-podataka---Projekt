...
select ime || ' ' || prezime as "Ime i prezime", datum_zaposlenja 
    from zaposlenik where floor(months_between(sysdate, datum_zaposlenja)/12) > 5;
...
select adresa, poštanski_broj, grad_opæina from lokacija
    where županija = 'Vukovarsko-srijemska županija';
...
select distinct ime || ' ' || prezime as "Ime i prezime"
    ...
    join usluga us using (usluga_id, ustanova_id) join ustanova u using (ustanova_id)
    join ustanova_lokacija using (ustanova_id) join lokacija using (lokacija_id)
    where županija = 'Osjeèko-baranjska županija';  

-- pronaæi ime kirurga i sve korisnike usluga kirurga na KBC-u Sestre milosrdnice
select distinct z.ime || ' ' || z.prezime as "Kirurg", k.ime || ' ' || k.prezime as "Korisnik"
    from zaposlenik z join korištenje_usluge using (zaposlenik_id) join korisnik k using (oib)
...
-- pronaæi prvih 10 korisnika, usluga, ustanova i datuma korištenja usluga cijene veæe ili jednake 500 kn
select k.ime || ' ' || k.prezime as "Korisnik", us.naziv, u.naziv, datum_usluge
    from korištenje_usluge ku join korisnik k on (ku.oib = k.oib)
    join usluga us using (usluga_id, ustanova_id) join ustanova u using (ustanova_id)
    where cijena >= 500 order by datum_usluge fetch first 10 rows only; 
...
select round(avg(count(ku.oib)),2) as "Prosjek" 
    from korištenje_usluge ku join korisnik k on (k.oib = ku.oib)
    group by ku.oib;

-- ukupni iznos cijena korištenja usluga za korisnike bez osiguranja
select sum(cijena) as "Ukupna cijena" from korištenje_usluge
...
select ime, prezime, email from 
    (select ime, prezime, email, nvl(sum(cijena), 0) as "Ukupna cijena"
    from korisnik left join korištenje_usluge using (oib)
    left join usluga using (ustanova_id, usluga_id) 
    group by ime, prezime, oib, email)
...
select ime, prezime, datum_zaposlenja 
    from ustanova join zaposlenik using (ustanova_id) where razina = 'Sekundarna'
intersect
select distinct ime, prezime, datum_zaposlenja
    from zaposlenik join korištenje_usluge using (zaposlenik_id);

-- ime i prezime zaposlenika koji su dali usluge ukupne cijene preko 1000 sortirano po prezimenu pa imenu
select ime, prezime from zaposlenik where zaposlenik_id in
    (select zaposlenik_id 
    from korištenje_usluge join usluga using (usluga_id, ustanova_id)
    group by zaposlenik_id having sum(cijena) > 1000)
    order by 2, 1;
...
comment on table lokacija is 'toèna lokacija ustanova';
